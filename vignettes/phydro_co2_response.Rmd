...
### Light-saturated beta for 368.9 ppm vs 614.2 ppm

```{r}
dat_acc_amz = tibble(co2 = c(368.9, 614.2)) %>% mutate(dat = purrr::map(.x=co2, .f = ~rphydro_analytical(tc, tc, ppfd_max, netrad, vpd, .x, pa, fapar, kphio, psi_soil, rdark, vwind, par_plant, par_cost, options))) %>% unnest_wider(dat)
  
  
dat_inst_amz = tibble(co2 =c(368.9, 614.2)) %>% left_join(dat_acc_amz %>% select(co2, vcmax25, jmax25)) %>% mutate(dat = purrr::pmap(.l=list(co2, vcmax25, jmax25), .f = ~rphydro_instantaneous_analytical(..2, ..3, tc, tc, 1500, netrad, vpd, ..1, pa, fapar, kphio, psi_soil, rdark, vwind, par_plant, par_cost, options))) %>% select(-vcmax25, -jmax25) %>% unnest_wider(dat) %>% mutate(a=a/2)

df.m1.amz <- dat_inst_amz %>% melt("co2")

dat_base1_amz = dat_inst_amz %>% filter(abs(co2-368) == min(abs(co2-368)) )

dat_inst_amz %>% 
  filter(co2 > 300) %>% 
  mutate(beta_a = log(a/dat_base1_amz$a)/log(co2/dat_base1_amz$co2),
         beta_vcmax = log(vcmax25/dat_base1_amz$vcmax25)/log(co2/dat_base1_amz$co2)
         ) 
```
